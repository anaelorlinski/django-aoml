...
class SMTPServerTestCase(TestCase):
    ...
    def test_credits(self):
        ...
        self.assertEqual(self.server.credits(), 10000)
        # Testing default limit
        self.server.mails_hour = 42
        self.assertEqual(self.server.credits(), 42)
        ...
        self.assertEqual(self.server.credits(), 41)
        ContactMailingStatus.objects.create(newsletter=self.newsletter,
                                            contact=self.contact,
                                            status=ContactMailingStatus.SENT_TEST)
        self.assertEqual(self.server.credits(), 40)
        ...
        ContactMailingStatus.objects.create(newsletter=self.newsletter_2,
                                            contact=self.contact,
                                            status=ContactMailingStatus.SENT)
        self.assertEqual(self.server.credits(), 39)
    ...
    def test_custom_headers(self):
        self.assertEqual(self.server.custom_headers, {})
        self.server.headers = 'key_1: val_1\r\nkey_2   :   val_2'
        self.assertEqual(len(self.server.custom_headers), 2)
...
class ContactTestCase(TestCase):
    ...
    def test_mail_format(self):
        ...
        self.assertEqual(contact.mail_format(), 'test@domain.com')
        ...
        self.assertEqual(contact.mail_format(), 'Titi Toto <test@domain.com>')

    def test_vcard_format(self):
        contact = Contact(email='test@domain.com', first_name='Toto', last_name='Titi')
        self.assertEqual(contact.vcard_format(), 'BEGIN:VCARD\r\nVERSION:3.0\r\n'\
                          'EMAIL;TYPE=INTERNET:test@domain.com\r\nFN:Toto Titi\r\n'\
                          'N:Titi;Toto;;;\r\nEND:VCARD\r\n')

    def test_subscriptions(self):
        contact = Contact.objects.create(email='test@domain.com')
        self.assertEqual(len(contact.subscriptions()), 0)

        self.mailinglist_1.subscribers.add(contact)
        self.assertEqual(len(contact.subscriptions()), 1)
        self.mailinglist_2.subscribers.add(contact)
        self.assertEqual(len(contact.subscriptions()), 2)

    def test_unsubscriptions(self):
        contact = Contact.objects.create(email='test@domain.com')
        self.assertEqual(len(contact.unsubscriptions()), 0)

        self.mailinglist_1.unsubscribers.add(contact)
        self.assertEqual(len(contact.unsubscriptions()), 1)
        self.mailinglist_2.unsubscribers.add(contact)
        self.assertEqual(len(contact.unsubscriptions()), 2)
...
class MailingListTestCase(TestCase):
    ...
    def test_subscribers_count(self):
        ...
        mailinglist.save()
        self.assertEqual(mailinglist.subscribers_count(), 0)
        mailinglist.subscribers.add(self.contact_1, self.contact_2, self.contact_3)
        self.assertEqual(mailinglist.subscribers_count(), 3)

    def test_unsubscribers_count(self):
        mailinglist = MailingList.objects.create(name='Test MailingList')
        self.assertEqual(mailinglist.unsubscribers_count(), 0)
        mailinglist.unsubscribers.add(self.contact_1, self.contact_2, self.contact_3)
        self.assertEqual(mailinglist.unsubscribers_count(), 3)

    def test_expedition_set(self):
        mailinglist = MailingList.objects.create(name='Test MailingList')
        self.assertEqual(len(mailinglist.expedition_set()), 0)
        mailinglist.subscribers.add(self.contact_1, self.contact_2, self.contact_3)
        self.assertEqual(len(mailinglist.expedition_set()), 1)
        mailinglist.subscribers.add(self.contact_4)
        self.assertEqual(len(mailinglist.expedition_set()), 2)
...
class NewsletterTestCase(TestCase):
    ...
    def test_mails_sent(self):
        self.assertEqual(self.newsletter.mails_sent(), 0)
        ContactMailingStatus.objects.create(newsletter=self.newsletter,
                                            contact=self.contact,
                                            status=ContactMailingStatus.SENT)
        ContactMailingStatus.objects.create(newsletter=self.newsletter,
                                            contact=self.contact,
                                            status=ContactMailingStatus.SENT_TEST)
        self.assertEqual(self.newsletter.mails_sent(), 1)
...
class TokenizationTestCase(TestCase):
    ...
    def test_tokenize_untokenize(self):
        ...
        self.assertEqual(untokenize(uidb36, token), self.contact)
...
class MailerTestCase(TestCase):
    ...
    def test_expedition_list(self):
        ...
        self.assertEqual(len(mailer.expedition_list), 2)
        self.server.mails_hour = 1
        self.assertEqual(len(mailer.expedition_list), 1)

        self.server.mails_hour = 100
        mailer = Mailer(self.newsletter)
        self.assertEqual(len(mailer.expedition_list), 4)
        self.server.mails_hour = 3
        self.assertEqual(len(mailer.expedition_list), 3)
    ...
    def test_run(self):
        ...
        mailer.run()
        self.assertEqual(mailer.smtp.mails_sent, 4)
        self.assertEqual(ContactMailingStatus.objects.filter(
        ...
        mailer.run()
        self.assertEqual(mailer.smtp.mails_sent, 2)
        self.assertEqual(ContactMailingStatus.objects.filter(
    ...
    def test_update_newsletter_status(self):
        mailer = Mailer(self.newsletter, test=True)
        self.assertEqual(self.newsletter.status, Newsletter.WAITING)
        ...
        self.assertEqual(self.newsletter.status, Newsletter.WAITING)
        mailer.update_newsletter_status()
        self.assertEqual(self.newsletter.status, Newsletter.SENDING)
    ...
    def test_recipients_refused(self):
        ...
        self.assertEqual(contact.valid, True)
        self.assertEqual(ContactMailingStatus.objects.filter(
        ...
        self.assertEqual(Contact.objects.get(email='thisisaninvalidemail').valid, False)
        self.assertEqual(ContactMailingStatus.objects.filter(
...
class StatisticsTestCase(TestCase):
    ...
    def test_get_newsletter_opening_statistics(self):
        ...
        self.assertEqual(stats['total_openings'], 0)
        self.assertEqual(stats['unique_openings'], 0)
        self.assertEqual(stats['double_openings'], 0)
        self.assertEqual(stats['unique_openings_percent'], 0)
        self.assertEqual(stats['unknow_openings'], 0)
        self.assertEqual(stats['unknow_openings_percent'], 0)
        self.assertEqual(stats['opening_average'], 0)
        ...
        self.assertEqual(stats['total_openings'], 4)
        self.assertEqual(stats['unique_openings'], 3)
        self.assertEqual(stats['double_openings'], 1)
        self.assertEqual(stats['unique_openings_percent'], 75.0)
        self.assertEqual(stats['unknow_openings'], 1)
        self.assertEqual(stats['unknow_openings_percent'], 25.0)
        self.assertEqual(stats['opening_average'], 1.3333333333333333)
        self.assertEqual(stats['opening_deducted'], 0)
        ...
        stats = get_newsletter_opening_statistics(status, self.recipients)
        self.assertEqual(stats['total_openings'], 5)
        self.assertEqual(stats['unique_openings'], 4)
        self.assertEqual(stats['double_openings'], 1)
        self.assertEqual(stats['unique_openings_percent'], 100.0)
        self.assertEqual(stats['unknow_openings'], 0)
        self.assertEqual(stats['unknow_openings_percent'], 0.0)
        self.assertEqual(stats['opening_average'], 1.25)
        self.assertEqual(stats['opening_deducted'], 1)

    def test_get_newsletter_on_site_opening_statistics(self):
        stats = get_newsletter_on_site_opening_statistics(self.status)
        self.assertEqual(stats['total_on_site_openings'], 0)
        self.assertEqual(stats['unique_on_site_openings'], 0)
        ...
        self.assertEqual(stats['total_on_site_openings'], 4)
        self.assertEqual(stats['unique_on_site_openings'], 3)
    ...
    def test_get_newsletter_clicked_link_statistics(self):
        ...
        self.assertEqual(stats['total_clicked_links'], 0)
        self.assertEqual(stats['total_clicked_links_percent'], 0)
        self.assertEqual(stats['double_clicked_links'], 0)
        self.assertEqual(stats['double_clicked_links_percent'], 0.0)
        self.assertEqual(stats['unique_clicked_links'], 0)
        self.assertEqual(stats['unique_clicked_links_percent'], 0)
        self.assertEqual(stats['clicked_links_by_openings'], 0.0)
        self.assertEqual(stats['clicked_links_average'], 0.0)
        ...
        self.assertEqual(stats['total_clicked_links'], 5)
        self.assertEqual(stats['total_clicked_links_percent'], 125.0)
        self.assertEqual(stats['double_clicked_links'], 2)
        self.assertEqual(stats['double_clicked_links_percent'], 50.0)
        self.assertEqual(stats['unique_clicked_links'], 3)
        self.assertEqual(stats['unique_clicked_links_percent'], 75.0)
        self.assertEqual(stats['clicked_links_by_openings'], 166.66666666666669)
        self.assertEqual(stats['clicked_links_average'], 1.6666666666666667)
    ...
    def test_get_newsletter_unsubscription_statistics(self):
        ...
        self.assertEqual(stats['total_unsubscriptions'], 0)
        self.assertEqual(stats['total_unsubscriptions_percent'], 0.0)
        ...
        self.assertEqual(stats['total_unsubscriptions'], 2)
        self.assertEqual(stats['total_unsubscriptions_percent'], 50.0)
    ...
    def test_get_newsletter_top_links(self):
        ...
        self.assertEqual(stats['top_links'], [])
        ...
        self.assertEqual(len(stats['top_links']), 2)
        self.assertEqual(stats['top_links'][0]['link'], self.links[0])
        self.assertEqual(stats['top_links'][0]['total_clicks'], 4)
        self.assertEqual(stats['top_links'][0]['unique_clicks'], 3)
        self.assertEqual(stats['top_links'][1]['link'], self.links[1])
        self.assertEqual(stats['top_links'][1]['total_clicks'], 1)
        self.assertEqual(stats['top_links'][1]['unique_clicks'], 1)
    ...
    def test_get_newsletter_statistics(self):
        ...
        self.assertEqual(stats['clicked_links_by_openings'], 0.0)
        self.assertEqual(stats['double_clicked_links'], 0)
        self.assertEqual(stats['double_clicked_links_percent'], 00.0)
        self.assertEqual(stats['double_openings'], 0)
        self.assertEqual(stats['mails_sent'], 4)
        self.assertEqual(stats['mails_to_send'], 4)
        self.assertEqual(stats['opening_average'], 0)
        self.assertEqual(stats['remaining_mails'], 0)
        self.assertEqual(stats['tests_sent'], 1)
        self.assertEqual(stats['top_links'], [])
        self.assertEqual(stats['total_clicked_links'], 0)
        self.assertEqual(stats['total_clicked_links_percent'], 0.0)
        self.assertEqual(stats['total_on_site_openings'], 0)
        self.assertEqual(stats['total_openings'], 0)
        self.assertEqual(stats['total_unsubscriptions'], 0)
        self.assertEqual(stats['total_unsubscriptions_percent'], 0.0)
        self.assertEqual(stats['unique_clicked_links'], 0)
        self.assertEqual(stats['unique_clicked_links_percent'], 0.0)
        self.assertEqual(stats['unique_on_site_openings'], 0)
        self.assertEqual(stats['unique_openings'], 0)
        self.assertEqual(stats['unique_openings_percent'], 0)
        self.assertEqual(stats['unknow_openings'], 0)
        ...
        self.assertEqual(stats['clicked_links_average'], 1.6666666666666667)
        self.assertEqual(stats['clicked_links_by_openings'], 100.0)
        self.assertEqual(stats['double_clicked_links'], 2)
        self.assertEqual(stats['double_clicked_links_percent'], 50.0)
        self.assertEqual(stats['double_openings'], 2)
        self.assertEqual(stats['mails_sent'], 4)
        self.assertEqual(stats['mails_to_send'], 4)
        self.assertEqual(stats['opening_average'], 1.6666666666666667)
        self.assertEqual(stats['remaining_mails'], 0)
        self.assertEqual(stats['tests_sent'], 1)
        self.assertEqual(stats['total_clicked_links'], 5)
        self.assertEqual(stats['total_clicked_links_percent'], 125.0)
        self.assertEqual(stats['total_on_site_openings'], 2)
        self.assertEqual(stats['total_openings'], 5)
        self.assertEqual(stats['total_unsubscriptions'], 1)
        self.assertEqual(stats['total_unsubscriptions_percent'], 25.0)
        self.assertEqual(stats['unique_clicked_links'], 3)
        self.assertEqual(stats['unique_clicked_links_percent'], 75.0)
        self.assertEqual(stats['unique_on_site_openings'], 2)
        self.assertEqual(stats['unique_openings'], 3)
        self.assertEqual(stats['unique_openings_percent'], 75)
        self.assertEqual(stats['unknow_openings'], 1)
        self.assertEqual(stats['unknow_openings_percent'], 25.0)
